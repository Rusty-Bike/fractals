...
/**
  ...
  */
final case class KochSegment(start: Vec2, dir: Vec2, length: Double, degrees: Double) extends FractalSegment {
  ...
  def divide: Seq[KochSegment] = {
    ...
    Seq(
      KochSegment(start, dir, newLength, degrees),
      KochSegment(start + dir * newLength, dir.rotate(-degrees), newLength, degrees),
      KochSegment(start + dir * newLength + dir.rotate(-degrees) * newLength, dir.rotate(degrees), newLength, degrees),
      KochSegment(start + dir * newLength  * (1 + 2 * cos), dir, newLength, degrees)
    )
  }
}
